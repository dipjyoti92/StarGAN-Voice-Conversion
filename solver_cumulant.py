    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if self.beta == 0:
                ...
                max_val = torch.max((-self.beta) * out_src)
                d_loss_real = (1.0 / self.beta) * (torch.log(torch.mean(torch.exp((-self.beta) * out_src - max_val))) + max_val)
            ...
            if self.gamma == 0:
                ...
                max_val = torch.max((self.gamma) * out_src)
                d_loss_fake = (1.0 / self.gamma) * (torch.log(torch.mean(torch.exp(self.gamma * out_src - max_val))) + max_val)
            ...
            if (i+1) % self.n_critic == 0:
                ...
                if self.gamma == 0:
                    ...
                    max_val = torch.max((self.gamma) * out_src)
                    g_loss_fake = -(1.0 / self.gamma) * (torch.log(torch.mean(torch.exp(self.gamma * out_src - max_val))) + max_val)
